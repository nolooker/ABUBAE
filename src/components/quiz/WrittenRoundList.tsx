...
export default function WrittenRoundList({ examSlug, rounds }: Props) {
            ...
            {rounds.filter((item) => item.year === year).map((item, index, items) => (
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-bold">{item.round}회 필기</h3>
                    <p className="mt-1 text-sm text-[var(--text-secondary)]">
                      {item.questionCount}문항 · {item.subjectCount}과목
                    </p>
                  </div>
                </div>
                <Link
                  href={`/exam/${examSlug}/questions/written/${item.year}/${item.round}`}
                  aria-label={`${item.year}년 ${item.round}회 문제 풀기`}

...
const valid = {
  ...
  blanks: [
    { blankNumber: 1, acceptedAnswers: ['106.00'] },
  ],
  expectedUpdatedAt: '2026-07-23T00:00:00Z',
}

function request(body: unknown = valid) {
  return new Request(`http://localhost/api/admin/practical-questions/${questionId}`, {
    method: 'PATCH',
...
describe('PATCH /api/admin/practical-questions/[questionId]', () => {
  beforeEach(() => {
  ...
  it('returns 400 for invalid ids, malformed JSON, and invalid edit payloads', async () => {
    ...
    const invalidPayload = await PATCH(request({ ...valid, blanks: [] }), context())
  ...
  it.each([
    ...
    [],
    {},
    { id: questionId, number: '3', subject: '실기', content: valid.content, updated_at: '2026-07-23T01:00:00Z' },
  ...
  it('uses the authenticated server client to atomically update and return the UI-safe result', async () => {
    ...
    expect(supabase.rpc).toHaveBeenCalledWith('update_practical_question', {
      p_question_id: questionId,
      p_content: valid.content,
      p_explanation: valid.explanation,
      p_blanks: [{ blank_number: 1, accepted_answers: ['106.00'] }],
  ...
  it.each([
    ...
    [{ code: 'P0001', message: 'practical question not found' }, 404],
